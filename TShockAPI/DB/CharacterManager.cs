			...
			try
			{
				using (var reader = database.QueryReader("SELECT * FROM tsCharacter WHERE Account=@0", acctid))
				{
					if (reader.Read())
					{
						...
						playerData.buffs = reader.Get<string>("buffs").Split('~').Select(NetBuff.Parse).ToList();
						return playerData;
					}
				}
			}
